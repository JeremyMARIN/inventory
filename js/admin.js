...
function processListData(data, listContent) {
	...
	for (var i = 0, length = data.length; i < length; i++) {
		...
		if (data[i].title != null && data[i].file != null && data[i].stock != null && data[i].price != null) {
			...
			line += "<div><label for=\"stock\">Stock: </label><input id=\"stock\" type=\"text\" name=\"" + data[i].id + "_stock\" value=\"" + data[i].stock + "\" size=\"3\" class=\"radius text-centered\" /></div>";
			line += "<div style=\"margin-top: 4px;\"><label for=\"price\">Price ($): </label><input id=\"price\" type=\"text\" name=\"" + data[i].id + "_price\" value=\"" + data[i].price + "\" size=\"3\" class=\"radius text-centered\" /></div>";
			line += "</div></div></div>";
